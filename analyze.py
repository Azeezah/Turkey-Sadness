...
def part_2a(df):
    ...
    fig.savefig(path_to_dir + '1989-2002_Values.png')
    print(".........Part 2a Plot Saved!\n")
    plt.close(fig)
    return df
...
def part_3b(line, df_2017):
    ...
    # format date objects into nice strings
    form_dates = [dt.datetime.strftime(date, '%b, %Y') for date in dates]
...
def part_3e(x, y, line):
    ...
    fig.savefig(path_to_dir + 'Line_Reg.png')
    print("\n.........Part 3e Plot Saved!"); plt.close(fig)

def to_float(x):
    '''helper function to remove commas from Value strings'''
    num = x.replace(",", "")
    return float(num)
